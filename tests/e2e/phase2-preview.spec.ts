...
test("phase2 preview: creator saves assembly and admin observes the same project preview", async ({ page }) => {
  ...
  await page.getByPlaceholder("输入 shotId，例如 shot-live-1").fill("shot-preview-2");
  await page.getByPlaceholder("可选，输入 primaryAssetId").fill("asset-preview-2");
  await page.getByPlaceholder("可选，输入 sourceRunId").fill("run-preview-2");
  ...
  await expect(
    page.locator('[data-testid^="preview-item-"]').first().locator("input").first(),
  ).toHaveValue("shot-preview-2");
  ...
  await expect(page.getByText("Shot ID：shot-preview-2")).toBeVisible();
