...
test("phase2 asset reuse: creator reuses an eligible external asset and admin audits the same shot", async ({
  ...
  await expect(page.getByText("asset-external-ai-1", { exact: true })).toBeVisible();
  await expect(
    page.getByText("policyapp: consent status must be granted for ai_annotated assets"),
  ).toBeVisible();
  ...
  await expect(blockedCard.getByText(/同意 unknown/)).toBeVisible();

  const allowedCard = page.getByText("asset-external-1", { exact: true }).locator("..");
  await expect(allowedCard.getByText(/同意 not_required/)).toBeVisible();
  ...
  await expect(creatorProvenanceDialog.getByText(/consentStatus=not_required/)).toBeVisible();
  await expect(creatorProvenanceDialog.getByText(/consent_status=not_required/)).toBeVisible();
  ...
  await expect(page.getByText(/consentStatus=not_required/)).toBeVisible();
  ...
  await expect(adminProvenanceDialog.getByText(/consentStatus=not_required/)).toBeVisible();
});
