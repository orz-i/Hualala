...
本轮 patch 默认不扩新的 public contract，不改 `proto/**`、generated SDK 或 session wire shape；允许变化的范围是：
...
- backend Connect contract suite 默认以当前 wire shape 为真相，不在测试重组时引入新的 proto 字段或 SDK 接口
...
1. 新增或修改 `proto/hualala/**/*.proto`
2. 刷新 `packages/sdk/src/gen/**` 或新增 SDK public API
