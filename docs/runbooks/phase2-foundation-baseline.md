...
当前 `master` 上的 foundation baseline 仍然以共享真相层治理为核心，但从 2026-03-23 这一轮开始，允许通过独立 foundation micro-patch 收口首批协同 / 预演 shared truth，以及后续音频时间线 shared truth。默认允许变化的范围是：
...
- 音频时间线 shared truth 的 proto、SDK client、backend persistence、asset truth 映射与 migration
...
- 音频时间线 shared truth 统一经由 `project_service.proto`、`asset.proto`、对应 SDK client 与 `0016_phase2_audio_timeline_shared_truth.sql` 收口
...
6. 需要通过 foundation micro-patch 新增音频时间线 shared truth，且范围限定在 `project_service.proto`、`asset.proto`、对应 SDK client、backend persistence 与 migration
