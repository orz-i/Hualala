...
因此这里的约束应理解为“默认少用 FK”，而不是“全库禁 FK 的不可突破原则”。对于治理域或低频强一致关系，若实现阶段证明 FK 更能降低错误面，可逐表评估。
...
正文版本化的最小实现通过 `content_snapshots` 承接。Phase 1 至少应支持 Story Bible / Script / Storyboard / Shot 相关正文快照，以支撑版本对比与回退。
...
`workflow_runs` 表达一条高价值长链路工作流实例，例如“剧本生成 -> 分镜拆解 -> 镜头执行治理链”。动态样片预演相关 workflow 在 Phase 1 只保留类型与资源绑定边界，不作为 MVP 硬验收前置。
...
create index if not exists idx_workflow_runs_resource_created
    ...
    where resource_id is not null;
create index if not exists idx_workflow_runs_execution_run
    on workflow_runs (shot_execution_run_id, created_at desc)
    where shot_execution_run_id is not null;
...
create index if not exists idx_usage_records_workflow_created
    ...
    where workflow_run_id is not null;
create index if not exists idx_usage_records_execution_run_created
    on usage_records (shot_execution_run_id, created_at desc)
    where shot_execution_run_id is not null;
...
create table if not exists billing_events (
    ...
    meta jsonb not null default '{}'::jsonb,
    created_at timestamptz not null default now(),
    constraint chk_billing_events_resource_type_non_empty check (length(trim(resource_type)) > 0),
...
create index if not exists idx_billing_events_workflow_created
    ...
    where workflow_run_id is not null;
create index if not exists idx_billing_events_execution_run_created
    on billing_events (shot_execution_run_id, created_at desc)
    where shot_execution_run_id is not null;
create index if not exists idx_billing_events_resource_created
    on billing_events (resource_type, resource_id, created_at desc)
    where resource_id is not null;
```
...
create table if not exists event_outbox (
    ...
    created_at timestamptz not null default now(),
    constraint chk_event_outbox_resource_type_non_empty
        check (length(trim(resource_type)) > 0),
...
create index if not exists idx_event_outbox_project_created
    ...
    where project_id is not null;
create index if not exists idx_event_outbox_resource_created
    on event_outbox (resource_type, resource_id, created_at desc)
    where resource_id is not null;
```
...
3. `asset_provenance_records / timeline_tracks / track_asset_bindings`
4. 如确有需要，再补 `asset_links / approvals / job_events` 等扩展表
