...
func (s *PostgresStore) SavePreviewRuntime(ctx context.Context, record project.PreviewRuntime) error {
	...
	`, record.ID, record.ProjectID, emptyToNil(record.EpisodeID), record.AssemblyID, defaultString(record.Status, "draft"), emptyToNil(record.RenderWorkflowRunID), defaultString(record.RenderStatus, "idle"), emptyToNil(record.PlaybackAssetID), emptyToNil(record.ExportAssetID), strings.TrimSpace(record.ResolvedLocale), emptyToNil(record.Playback.DeliveryMode), emptyToNil(record.Playback.PlaybackURL), emptyToNil(record.Playback.PosterURL), nullableInt(record.Playback.DurationMs), emptyToNil(record.ExportOutput.DownloadURL), emptyToNil(record.ExportOutput.MimeType), emptyToNil(record.ExportOutput.FileName), nullableInt64(record.ExportOutput.SizeBytes), emptyToNil(record.LastErrorCode), emptyToNil(record.LastErrorMessage), record.CreatedAt, record.UpdatedAt)
	if err != nil {
