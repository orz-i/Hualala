...
func (p *PostgresPersister) loadPreviewRuntimes(ctx context.Context, snapshot *Snapshot) error {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&record.ID, &record.ProjectID, &record.EpisodeID, &record.AssemblyID, &record.Status, &record.RenderWorkflowRunID, &record.RenderStatus, &record.PlaybackAssetID, &record.ExportAssetID, &record.ResolvedLocale, &record.Playback.DeliveryMode, &record.Playback.PlaybackURL, &record.Playback.PosterURL, &record.Playback.DurationMs, &playbackTimelineText, &record.ExportOutput.DownloadURL, &record.ExportOutput.MimeType, &record.ExportOutput.FileName, &record.ExportOutput.SizeBytes, &record.LastErrorCode, &record.LastErrorMessage, &record.CreatedAt, &record.UpdatedAt); err != nil {
		...
		if err != nil {
			return fmt.Errorf("db: decode preview runtime timeline: %w", err)
		}
...
func (p *PostgresPersister) savePreviewRuntimes(ctx context.Context, tx *sql.Tx, snapshot Snapshot) error {
	for _, record := range snapshot.PreviewRuntimes {
		...
		if err != nil {
			return fmt.Errorf("db: encode preview runtime timeline %s: %w", record.ID, err)
		}
		...
		`, record.ID, record.ProjectID, emptyToNil(record.EpisodeID), record.AssemblyID, defaultString(record.Status, "draft"), emptyToNil(record.RenderWorkflowRunID), defaultString(record.RenderStatus, "idle"), emptyToNil(record.PlaybackAssetID), emptyToNil(record.ExportAssetID), record.ResolvedLocale, emptyToNil(record.Playback.DeliveryMode), emptyToNil(record.Playback.PlaybackURL), emptyToNil(record.Playback.PosterURL), nullableInt(record.Playback.DurationMs), playbackTimeline, emptyToNil(record.ExportOutput.DownloadURL), emptyToNil(record.ExportOutput.MimeType), emptyToNil(record.ExportOutput.FileName), nullableInt64(record.ExportOutput.SizeBytes), emptyToNil(record.LastErrorCode), emptyToNil(record.LastErrorMessage), record.CreatedAt, record.UpdatedAt); err != nil {
