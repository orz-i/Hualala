export type CollaborationOwnerType = "project" | "shot";

export type CollaborationPresenceViewModel = {
  presenceId: string;
  sessionId: string;
  userId: string;
  status: string;
...
export type CollaborationSessionViewModel = {
  session: {
    sessionId: string;
    ownerType: string;
    ownerId: string;
  ...
  presences: CollaborationPresenceViewModel[];
  scope: {
...
export function isLeaseStale(leaseExpiresAt: string) {
  if (!leaseExpiresAt) {
    return false;
  }
