...
AI 剧集生成平台 monorepo。当前 `master` 已完成 Phase 1 主闭环，并把 mock + real 两条验收门接入仓库工作流。

## 仓库组成

- `apps/backend`：Go Connect API、SSE、Upload Session、内存态 Phase 1 backend
- `apps/admin`：治理端工作台
- `apps/creator`：创作端工作台
...
- 入口说明：`docs/runbooks/phase1-demo.md`
- 主命令：`corepack pnpm run demo:seed:backend`
